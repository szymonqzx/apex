...
public class ApexAgentDaemon {
  ...
  public void start() {
    ...
    // Set OOM adjustment — killable under memory pressure (hidden API, via reflection)
    try {
      Class<?> processClass = Class.forName("android.os.Process");
      java.lang.reflect.Method setOomAdj = processClass.getDeclaredMethod("setOomAdj", int.class, int.class);
      setOomAdj.invoke(null, android.os.Process.myPid(), 900);
    } catch (Exception e) {
  ...
  private void startSocketServer() {
    try {
      LocalServerSocket server = new LocalServerSocket(SOCKET_PATH);
  ...
  private void handleClient(android.net.LocalSocket client) {
    try {
